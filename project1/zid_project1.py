...
def get_tics(pth):
    ...
    # opens the file at 'pth' in read mode
    with open(pth, "r") as file:
        for line in file:
            # splits the exchange and tic by the "="
            text = line.split("=")

            # filtering white space, lower case, and non-alphabetical characters
            exchange = text[0].strip().lower().replace('"', "")
            tic = text[1].strip().lower().replace('"', "")

            # puts new tickers and exchanges into a dictionary
            dict_get_tics.update({tic: exchange})

    return dict_get_tics
...
def read_dat(tic):
    ...
    with open(file_path, "r") as file:
        for line in file:
            # adds string to the list while removing the "\n"
            read_dat_list.append(line.strip())

    return read_dat_list
...
def line_to_dict(line):
    ...
    # creates a starting value for the for loop
    start = 0

    for col in COLUMNS:
        # cycles through each value of COLUMNS
        value = line[start:COLWIDTHS[col] + start]

        # slices based on the COLWIDTH
        start = start + COLWIDTHS[col]

        # adds the value to final_dict
        final_dict.update({col: value})
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    """
    # checking if the tickers_lst is None
    if tickers_lst is not None:
        if len(tickers_lst) == 0:
            # tickers_lst is an empty list
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    # Checking for invalid ticker names
    verify_tickers(tic_exchange_dic, tickers_lst)
    if tickers_lst is None:
        # returns a list formed from tic_exchange_dic
        tickers_lst = tic_exchange_dic.keys()

    # Creates an empty dictionary
    final_dict = {}

    # Cycling through all the tickers in tickers_lst
    for ticker in tickers_lst:
        # Extracts the stock exchange
        exchange = tic_exchange_dic[ticker]

        # Extracts stock data
        stock_data = read_dat(ticker)

        # Creating an empty list to store the filtered data
        filtered_data_list = []

        # Moving stock_data into a dictionary
        for line in stock_data:

            # Putting the values of each line into a dictionary
            stock_data_dict = line_to_dict(line)

            # Creating an empty dict to store filtered data from each line
            filtered_data_dict = {}

            # Create a filter that only takes the specified columns
            for col in col_lst:

                # Adding the specified column into the filtered dict col_lst
                filtered_data_dict[col] = stock_data_dict[col].strip()

            # Adding the filtered_dict into the filtered list
            filtered_data_list.append(filtered_data_dict)
        ...
        final_dict[ticker] = final_filtered_dict
...
if __name__ == "__main__":
    # Test functions
    _test_get_tics()
    # _test_read_dat() # reads a line from a ticker
    # _test_line_to_dict() # returns the line from a ticker and returns it as a dictionary
